...
from utils import GaussianBlur3d
...
class meta_model(nn.Module):
    ...
    def __init__(self, l, im_shape, device, kernel_size, sigma_v, mu, z0, h=20):
        super().__init__()
        self.l = l
        self.res_list = []
        self.device = device
        ...
        #self.id_grid = K.utils.grid.create_meshgrid3d(im_shape[2], im_shape[3], im_shape[4], False, device)
        self.id_grid = create_meshgrid3d(im_shape[2], im_shape[3], im_shape[4], device=device, dtype=torch.HalfTensor)
        #self.kernel = GaussianBlur3d(kernel_size, sigma_v)
...
class meta_model_local(nn.Module):
    ...
    def __init__(self, l, im_shape, device, kernel_size, sigma_v, mu, z0, h=20):
        ...
        self.z0 = nn.Parameter(z0)
        #self.id_grid = K.utils.grid.create_meshgrid3d(im_shape[4], im_shape[3], im_shape[2], False, device)
...
class meta_model_local_sharp(nn.Module):
    ...
    def __init__(self, l, im_shape, device, kernel_size, sigma_v, mu, z0, h=50):
        super().__init__()
        self.l = l
        self.res_list = []
        self.device = device
...
"""class meta_model_downsample(nn.Module):

    def __init__(self, l, im_shape, device, kernel_size, sigma_v, mu, z0, h=10):
        super().__init__()
        self.l = l
        self.res_list = [res_block(h, layer="first")]
        self.device = device
        for i in range(l-2):
            self.res_list.append(res_block(h))
        self.res_list.append(res_block(h, layer="last"))
        self.res_list = nn.ModuleList(self.res_list)
    ...
    def forward(self, source, source_seg):
        ...
        for i in range(self.l):
            ...
            self.field[i] = self.field[i].permute(0,4,3,2,1)
            deformation = self.id_grid - self.field[i] / self.l
            f = checkpoint(self.res_list[0], self.residuals[i], image) * 1 / self.l
            self.residuals.append(self.residuals[i] + f)

            residuals_deformed = [deform_image(z, deformation) for z in residuals_deformed]
            residuals_deformed.append(self.residuals[-1])
            self.phi.append(deform_image(self.phi[-1].permute(0,4,3,2,1), deformation).permute(0,4,3,2,1))
